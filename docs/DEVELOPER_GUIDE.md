...
## Iraq-specific localization and currency

- This application is targeted for Iraq. All prices in the prototype and UI should be in Iraqi Dinar (IQD), commonly shown as "د.ع" or "IQD".
- Use the built-in Iraqi currency formatter at `lib/core/utils/functions/currency_formatter_iraqi.dart` when displaying prices to ensure proper separators and locale conventions.
- Mock data should reflect Iraq-appropriate content: vendor names, product categories, and images that make sense for Iraqi users (home goods, local apparel, crafts, furniture, etc.).
- When writing mock prices in the repository or example JSON, use realistic IQD amounts (for example: a small home item might be 20,000 د.ع, furniture could be 200,000 د.ع). Avoid using USD-style decimals in display; use the formatter which rounds/group appropriately.

Example usage in code:
```dart
// format price
final formatted = CurrencyFormatterIraqi.format(20000); // -> "20,000 د.ع" (example)
```

Notes:
- The prototype uses local mock data in `lib/features/marketplace/data/repository/marketplace_repository.dart`. Update those sample items to IQD values and Iraqi product names/images as needed.
- For UX text (Arabic/Kurdish/English), reuse `assets/translations/*.json` and `easy_localization` already configured in `main.dart`.
...
## Running the Marketplace Prototype (quick)

Where I added files:
- `lib/features/marketplace/` — models, repository, cubits, and screens:
   - `data/model/vendor_model.dart`, `data/model/product_model.dart`
   - `data/repository/marketplace_repository.dart` (mock in-app JSON)
   - `cubit/` — `vendors_cubit.dart`, `storefront_cubit.dart`, `cart_cubit.dart` and states
   - `screen/` — `vendors_list_screen.dart`, `storefront_screen.dart`, `product_detail_screen.dart`, `cart_screen.dart`, `checkout_screen.dart`, `success_screen.dart`

How to open the prototype UI:
- Option A (recommended): Add a navigation entry or button in an existing screen (e.g., `lib/core/ui/screens/splash_screen.dart` or `features/home/screen`) that pushes `VendorsListScreen()`.
- Option B (dev quick run): Replace the home widget in `lib/main.dart` temporarily with a `MaterialApp(home: VendorsListScreen())` (remember to import the screen). Revert after testing.

DI: I registered `MarketplaceRepository` and `CartCubit` in `core/di/injection.dart` as lazy singletons so you can obtain them via `getIt<MarketplaceRepository>()`.

Notes:
- This is a mobile-first mock/prototype: no backend, no payments. Data is mocked in `MarketplaceRepository` using local asset image paths.
- If you want, I can wire the prototype into the real `main.dart` flows (add a menu item or home CTA) and polish the UI to match a modern, beautiful design (animations, nicer product cards, hero images).
